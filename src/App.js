...
// ── [설정 창] 나중에 이 숫자만 바꾸면 앱이 자동으로 업데이트됨 ────────────────
  ...
  return (
    <div style={{background:"#0c0c18",border:"1px solid #1a1a2e",borderRadius:9, marginBottom:7,borderLeft:`3px solid ${c.color}`,overflow:"hidden"}}>
      <div style={{padding:"12px 13px"}}>
        ...
        {open&&( <div style={{marginTop:9,fontSize:11,color:"#6060a0",lineHeight:1.6}}>{c.signal}</div> )}
      </div>
    </div>
  );
}
...
function AlgoFlow() {
  ...
  return (
    <div style={{background:"#0c0c18",border:"1px solid #1a1a2e",borderRadius:12, padding:"14px 13px",marginBottom:14}}>
      {isOverride && <div style={{background:"rgba(255,59,59,0.12)",border:"1px solid #ff3b3b",borderRadius:8,padding:"10px",marginBottom:12,color:"#ff3b3b",fontWeight:900}}>⚡ OVERRIDE: 매크로 무시 풀파워 매수</div>}
      {steps.map((s)=>(
        <div key={s.id} style={{display:"flex",gap:11,marginBottom:8}}>
          ...
          <div style={{flex:1,background:"#0a0a16",padding:"9px",borderRadius:8,borderLeft:`2px solid ${s.roleCol}`}}>
            <div style={{fontSize:13,fontWeight:700,color:"#d0d0e8"}}>{s.title} <span style={{fontSize:9,color:s.roleCol}}>[{s.role}]</span></div>
    ...
    <div style={{background:"#07070e",minHeight:"100vh",padding:"20px",color:"#d0d0e8",fontFamily:"sans-serif"}}>
      <div style={{maxWidth:460,margin:"0 auto"}}>
        <div style={{marginBottom:20}}>
          ...
          <div style={{fontSize:11,color:"#444460",fontFamily:"monospace"}}>LAST UPDATED: 2026.03.30</div>
        </div>
        <div style={{background:"#0c0c18",padding:16,borderRadius:12,marginBottom:14,display:"flex",gap:14,alignItems:"center"}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:macro.col}}>권장 속도: {macro.interval} 간격</div>
          </div>
        </div>
        <AlgoFlow/>
